                        ...
                        <tbody>
                            {reqLoading && requests.length === 0 ? (
                                ...
                                <tr key={req.id} onClick={() => router.push(`/admin/services/${req.id}`)} className="group cursor-pointer border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors relative">
                                    ...
                                    <td className="px-6 py-5">{req.assignedTo ? (<div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center overflow-hidden border border-blue-500/30">{req.assignedTo.profilePhoto ? <img src={req.assignedTo.profilePhoto} alt="" className="w-full h-full object-cover" /> : <User className="w-4 h-4 text-blue-400" />}</div><span className="text-[13px] font-bold text-gray-300">{req.assignedTo.user?.name || 'Staff Member'}</span></div>) : (<div className="flex items-center gap-3 text-gray-600"><div className="w-8 h-8 rounded-full bg-white/[0.03] border border-white/[0.05] flex items-center justify-center"><Users className="w-4 h-4" /></div><span className="text-[13px] font-medium">Unassigned</span></div>)}</td>
